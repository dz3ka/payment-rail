...
[![CI](https://github.com/dz3ka/payment-rail/actions/workflows/ci.yml/badge.svg?branch=master)](https://github.com/dz3ka/payment-rail/actions/workflows/ci.yml)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)

> ⚠️ **Testnet only.** Payment Rail handles **testnet funds only** and is **not
> audited**. It is a reference implementation of payments-grade engineering —
...
- **Double-entry ledger + idempotent payments API**: balances derived, never
  stored; every create commits as one atomic Postgres transaction; idempotency-key
  replay safety.
- **Isolated signer + EVM chain adapter**: keys never leave the signer;
  EIP-1559 USDC transfers behind a gap-free, chain-authoritative nonce allocator.
- **Reorg-safe settlement**: poll-based confirmation-depth tracking, reorg
  detection, and settlement effects that stay correct across chain reorgs and
  restarts.
- **Event backbone**: transactional outbox → Kafka/Redpanda (at-least-once) →
  `webhookd` HMAC-SHA256-signed delivery with exponential backoff, dead-lettering,
  and replay.
- **Compliance controls**: policy engine + denylist screening, per-signing-key
...
- **Reconciliation**: on-chain treasury balances vs. the ledger, with a
  proof-of-reserves report and a tri-state operator exit code.
- **Reliability evidence**: an in-process chaos / fault-injection suite (crash,
  DB failover, broken RPC, reorg) and a published
  [load-test benchmark](docs/benchmark.md).

See the [ADRs](docs/adr/) for the decisions behind each subsystem.

## Architecture at a glance

Seven binaries in one module (`cmd/` layout), external transport REST. Internal
transport is gRPC as the target end state (ADR-0001), but today the `api` runs
...
Kafka (Redpanda in dev) carries domain events.

| Binary | Role |
|--------|------|
| `api` | External REST payments API (idempotent create/get/list/cancel) |
| `ledger` | Scaffold entrypoint; the ledger runs in-process inside `api` — see ADR-0006 |
...
docs/
  ...
  learning/     design notes and rationale, per subsystem
  benchmark.md  published load-test benchmark
  threat-model.md
```

## Scope & limitations

Payment Rail is testnet-only and unaudited. Deliberately not built:

- **No internal gRPC transport yet** — the target topology (ADR-0001) puts
  services behind gRPC; today the `api` runs the ledger in-process instead,
  with no network hop on the payments path (ADR-0006).
- **`ledger` is a scaffold entrypoint**, not a working service — the ledger
  runs in-process inside `api` (see the component table above).
- **No mainnet adapter** — the EVM chain adapter only ever talks to testnet.
- **No Dockerfile or published image** — the dev stack is Docker Compose for
  infrastructure plus locally built binaries.

See [`docs/adr/`](docs/adr/) for the reasoning behind each of these boundaries.
