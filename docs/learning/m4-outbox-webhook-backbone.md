...
Debezium/connect cluster to operate, and makes the "what exactly is the public event
contract" question implicit. The application outbox keeps the envelope an explicit,
versioned artifact (`schema_version`, a curated `paymentEvent` body that deliberately
omits `journal_entry_id`) that evolves independently of the tables.
